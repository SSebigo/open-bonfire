...
class _SignUpWithEmailPageState extends State<SignUpWithEmailPage> with TickerProviderStateMixin {
  ...
  void _onConfirmPasswordChanged() {
    BlocProvider.of<AuthBloc>(context).add(
        OnConfirmPasswordChanged(password: _passwordController.text, confirmPassword: _confirmPasswordController.text));
  }

  void _signUp() {
    BlocProvider.of<AuthBloc>(context)
        .add(OnSignUpClicked(email: _emailController.text, password: _passwordController.text));
  }
        ...
        child: BlocListener<AuthBloc, AuthState>(
          ...
          listener: (BuildContext context, AuthState state) {
            if (state is SendMailSuccessState) {
              sailor.navigate(Constants.authConfirmationRoute, navigationType: NavigationType.pushReplace);
            }
          ...
          child: BlocBuilder<AuthBloc, AuthState>(
            ...
            builder: (BuildContext context, AuthState state) {
              if (state is SendingMailState || state is ConvertingAccountState) {
                return BonfireLoadingScreen(
                  vsync: this,
                  isLoading: state is SendingMailState || state is ConvertingAccountState,
                );
              }
              return Form(
                    ...
                    children: <Widget>[
                            ...
                            validator: (_) {
                              if (state is ConfirmPasswordValidityState) {
                                return !state.isConfirmPasswordValid
                                    ? I18n.of(context).textInvalidConfirmPassword
                                    : null;
                              }
                              return null;
                            },
                      ...
                      Padding(
                        ...
                        child: Button(
                          ...
                          onPressed: _isSignUpEnabled() == true ? () => _signUp() : null,
                        ),
                      )
                    ],
                  ),
