...
class MyProfileBloc extends Bloc<MyProfileEvent, MyProfileState> {
  ...
  @override
  MyProfileState get initialState => InitialMyProfileState();
  ...
  Stream<MyProfileState> mapEventToState(
    ...
    if (event is EVTOnProfilePictureClicked) {
      ...
      try {
        final String profilePictureUrl = await _onlineStorageRepository
            ?.uploadFile(event.file, Paths.profilePicturePath);
  ...
  Stream<MyProfileState> _mapOnFetchUserBonfiresEventToState(
      EVTOnFetchUserBonfires event) async* {
    ...
    try {
      ...
      _bonfireSubscription = _bonfireRepository.getUserBonfires(uid).listen(
          (bonfires) => add(EVTOnReceivedUserBonfires(bonfires: bonfires)));
    } catch (error) {
      yield MyProfileErrorState(error: error);
    }
  }

  Stream<MyProfileState> _mapOnReceivedUserBonfiresEventToState(
      EVTOnReceivedUserBonfires event) async* {
    try {
  ...
  Stream<MyProfileState> _mapOnUnfollowClickedToState(
  ...
  Stream<MyProfileState> _mapOnFetchFollowingDetailsToState(
      EVTOnFetchFollowingDetails event) async* {
