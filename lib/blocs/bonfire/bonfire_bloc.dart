...
import 'package:flutter/src/widgets/image.dart' as dart_image;
...
class BonfireBloc extends Bloc<BonfireEvent, BonfireState> {
  ...
  BonfireBloc() : super(InitialBonfireState());
  ...
  Stream<BonfireState> mapEventToState(
    ...
    if (event is EVTOnNavigateClicked) {
      yield STENavigating();
      try {
        final dart_image.ImageProvider imageProvider = dart_image.NetworkImage(event.bonfire.imageUrl);
        final ui.Image image = await getImageFromProvider(imageProvider);
        final color = await getColorFromImage(image);
        final ui.Color rgbaColor = ui.Color.fromRGBO(color[0] as int, color[1] as int, color[2] as int, 1.0);
  ...
  Stream<BonfireState> _mapOnDownloadFileClickedToState(EVTOnDownloadFileClicked event) async* {
    yield STEDownloadingFile();
    try {
      final String taskId = await downloadFile(event.fileUrl);
      if (taskId == null) {
        yield BonfireErrorState(error: DownloadFailedException(event.errorMessage));
      } else {
  ...
  Stream<BonfireState> _mapOnBonfireLikedToState(EVTOnBonfireLiked event) async* {
  ...
  Stream<BonfireState> _mapOnBonfireDislikedToState(EVTOnBonfireDisliked event) async* {
    yield STEBonfireUpdating();
    try {
      final List<String> likes = List<String>.from(event.likes);
      final List<String> dislikes = List<String>.from(event.dislikes);
  ...
  Stream<BonfireState> _mapOnFollowClickedToState(EVTOnFollowClicked event) async* {
    yield STEAddingFollowing();
    try {
      final int followingCount =
          _localStorageRepository?.getUserSessionData(Constants.sessionFollowing)?.length as int ?? 0;

      if (followingCount >= 100) {
        yield BonfireErrorState(error: FollowingLimitReachedException(event.errorMessage));
      } else {
        ...
        yield STEFollowingAdded(isFollowing: following?.contains(event.followingUid));
      }
    } catch (error) {
      yield BonfireErrorState(error: error);
    }
  ...
  Stream<BonfireState> _mapOnUnfollowClickedToState(EVTOnUnfollowClicked event) async* {
    ...
    try {
      ...
      final List<String> following =
          _localStorageRepository?.getUserSessionData(Constants.sessionFollowing)?.cast<String>() as List<String>;
      yield STEFollowingRemoved(isFollowing: following?.contains(event.unfollowingUid));
    } catch (error) {
      yield BonfireErrorState(error: error);
    }
  }

  Stream<BonfireState> _mapOnFetchBonfireUserDetailsToState(EVTOnFetchBonfireUserDetails event) async* {
    yield STEFetchingBonfireUserDetails();
    try {
      final BonfireUserDetails details = await _userDataRepository?.getUserByUid(
          event.authorUid, UserDataType.BONFIRE_USER_DETAILS) as BonfireUserDetails;
