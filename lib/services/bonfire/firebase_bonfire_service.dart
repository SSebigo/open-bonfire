...
class FirebaseBonfireService extends BaseBonfireService {
  ...
  static final FirebaseBonfireService _singleton = FirebaseBonfireService._internal();
  ...
  Stream<List<Bonfire>> getBonfires(Position position, double radius) {
    ...
    final GeoFirePoint center = geo.point(latitude: position.latitude, longitude: position.longitude);

    final CollectionReference bonfireCollection = _firestore.collection(Paths.bonfiresPath);
    ...
    */
    return geo
        ...
        .transform(StreamTransformer<List<DocumentSnapshot>, List<Bonfire>>.fromHandlers(
            handleData: (List<DocumentSnapshot> documents, EventSink<List<Bonfire>> sink) =>
  ...
  Stream<List<Bonfire>> getUserBonfires(String sessionUid) {
    return _firestore
        ...
        .transform(StreamTransformer<QuerySnapshot, List<Bonfire>>.fromHandlers(
            handleData: (QuerySnapshot querySnapshot, EventSink<List<Bonfire>> sink) =>
  ...
  @override
  Future<void> updateBonfireRating(String id, List<String> likes, List<String> dislikes) async {
    final DocumentReference ref = _firestore.collection(Paths.bonfiresPath).document(id);
    await ref.setData({'likes': likes, 'dislikes': dislikes}, merge: true);
  }

  @override
  Future<void> updateBonfireExpirationDate(String id, int hours, int mode) async {
    final DocumentReference ref = _firestore.collection(Paths.bonfiresPath).document(id);
    ...
    final DateTime initialExpirationDate = DateTime.fromMicrosecondsSinceEpoch(createdAt) + 1.days;
    ...
    if (mode == 0) {
      ...
      if ((expiresAtAsDate - hours.hours).millisecondsSinceEpoch >= initialExpirationDate.millisecondsSinceEpoch) {
        newExpirationDate = expiresAtAsDate - hours.hours;
      } else {
